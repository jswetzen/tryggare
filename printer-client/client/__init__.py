...
with WeasyPrint + pymupdf, and sends to a Brother QL or Dymo label printer.
...
LABEL_SIZE = os.environ.get(
    "LABEL_SIZE", "29x90"
)  # die-cut: 29x90, 62x100 | endless: 29, 62 (Dymo: 30252, 30334, 30256, 4xl)
PRINTER_TYPE = os.environ.get("PRINTER_TYPE", "brother")  # brother | dymo
...
_backend = get_backend(PRINTER_TYPE)
...
# ─────────────────────────────────────────────────────────────────────────────
# Printing — delegates to the configured backend (brother.py or dymo.py)
# ─────────────────────────────────────────────────────────────────────────────


def validate_label_size() -> None:
    """Fail fast if LABEL_SIZE isn't a known label identifier for the backend.

    A wrong value otherwise prints mis-sized garbage or fails obscurely deep in
    a print job.
    """
    _backend.validate_label_size(LABEL_SIZE)


def get_label_target_size() -> tuple[int, int]:
    """
    Return the expected (width, height) in pixels for the rendered label
    based on LABEL_SIZE, per the configured backend.
    """
    return _backend.get_label_target_size(LABEL_SIZE)


def print_label(png_bytes: bytes) -> None:
    """Send PNG bytes to the printer via the configured backend."""
    _backend.print_png(png_bytes)
...
# ─────────────────────────────────────────────────────────────────────────────
# Startup checks — delegate to the configured backend
# ─────────────────────────────────────────────────────────────────────────────
...
def check_ipp_usb() -> None:
    ...
    Brother-specific (ipp-usb conflicts with brother_ql's own USB access); other
    backends don't implement this hook.
    """
    if hasattr(_backend, "check_ipp_usb"):
        _backend.check_ipp_usb()
...
def discover_and_check_printer() -> str:
    ...
    PRINTER_IDENTIFIER = _backend.discover_printer(PRINTER_BACKEND, PRINTER_IDENTIFIER)
...
async def main(non_interactive: bool = False) -> None:
    log.info(
        "Printer client starting — type=%s, model=%s, label=%s, backend=%s",
        PRINTER_TYPE,
        PRINTER_MODEL,
        LABEL_SIZE,
        PRINTER_BACKEND,
