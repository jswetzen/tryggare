...
>
  <!-- Family Header -->
  <div class="bg-neutral-50 p-2.5 sm:p-3 border-b border-neutral-200">
    <div class="flex items-center justify-between gap-2">
      <div class="flex-1 min-w-0">
        <h3 class="font-bold text-primary-900 text-base sm:text-lg">
          {family.name}
        </h3>
      ...
      <button
        ...
        class="px-3 py-1.5 sm:px-4 sm:py-2 bg-danger-600 text-white font-semibold rounded-button hover:bg-danger-700 transition-colors text-xs sm:text-sm whitespace-nowrap"
  ...
  <div class="p-2 sm:p-3 space-y-2">
    {#each family.children as child (child.id)}
      <div
        class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2 bg-neutral-50 rounded border border-neutral-200"
        data-testid={`child-checkout-row-${child.id}`}
      >
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 flex-wrap">
            <span class="font-medium text-neutral-700 text-sm sm:text-base">
              {child.firstName} {child.lastName}
            </span>
            {#if child.supervised}
              <span class="px-1.5 py-0.5 text-xs font-semibold bg-primary-100 text-primary-800 rounded">
        ...
        <button
          ...
          class="px-3 py-1.5 bg-danger-600 text-white font-semibold rounded-button hover:bg-danger-700 transition-colors text-xs sm:text-sm whitespace-nowrap self-start sm:self-center"
  ...
  <div class="p-2.5 sm:p-3 bg-neutral-50 border-t border-neutral-200">
    <label class="flex flex-col sm:flex-row sm:items-center gap-2">
      ...
      <select
        ...
        class="px-3 py-1.5 border border-neutral-300 rounded-input text-sm bg-white text-neutral-700 flex-1 focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
